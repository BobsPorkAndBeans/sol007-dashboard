...
async function loadReturns(){
  ...
  let history = [];
  try {
    const spark = await json('data/returns_sparkline.json');
    history = spark.points || [];
  } catch(err) { console.warn('returns sparkline unavailable', err); }
  ...
  const sparkEl = document.getElementById('returnsSparkline');
  const drawn = sparkline(history);
  sparkEl.textContent = drawn;
  sparkEl.hidden = !drawn;
}
async function main(){
  ...
  document.getElementById('tripwires').innerHTML = Object.entries(tw).map(([id,t]) => {
    const raw = (typeof t === 'object' && t) ? (t.status || 'unknown') : t;
    const note = (typeof t === 'object' && t && t.note) ? String(t.note) : '';
    const status = (raw === 'ok' && /unavailable/i.test(note)) ? 'unknown' : raw;
    return `<article class="tripwire"><strong>${id}</strong><div>${(t && t.label) || id}</div><span class="status">${statusText(status)}</span><p>${note}</p></article>`;
  }).join('');
